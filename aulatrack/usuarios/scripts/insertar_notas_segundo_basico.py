...
def generar_notas(curso, nombre_asignatura, rango=(4.8, 6.5)):
...
def run():
    curso = Curso.objects.filter(nombre__icontains="Segundo", año=2025).first()
    if not curso:
        print(" No se encontró el curso 2º Segundo Básico.")
        return

    total = 0

    # === Asignaturas de 2º Básico ===
    total += generar_notas(curso, "Ciencias Naturales", (4.8, 6.6))
    total += generar_notas(curso, "Educación Artística", (5.0, 6.8))
    total += generar_notas(curso, "Educación Física y Salud", (5.5, 6.9))
    total += generar_notas(curso, "Historia, Geografía y Ciencias Sociales", (4.7, 6.4))
    total += generar_notas(curso, "Inglés", (4.9, 6.5))
    total += generar_notas(curso, "Lenguaje y Comunicación", (4.6, 6.3))
    total += generar_notas(curso, "Matemáticas", (4.8, 6.7))
    total += generar_notas(curso, "Tecnología", (5.3, 6.8))
